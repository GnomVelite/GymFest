...
$sql = "SELECT * FROM `events` WHERE ownerid = ".$user->id." ORDER BY startDate ASC";
...
if ($result->num_rows > 0) {
  while($brow = $result -> fetch_assoc()) {
    $name = mb_convert_encoding($brow["name"], "UTF-8");
    if ($brow["endDate"] < time()){
      $endedEvents[$j]["eventName"] = $name;
      $endedEvents[$j]["eventId"] = $brow["id"];
      $endedEvents[$j]["startDate"] = $brow["startDate"];
      $endedEvents[$j]["startDate"] = $brow["startDate"];
      $endedEvents[$j]["endDate"] = $brow["endDate"];
      $endedEvents[$j]["publishedAt"] = $brow["publishedAt"];
      $endedEvents[$j]["price"] = $brow["price"];
      $endedEvents[$j]["tickets"] = $brow["tickets"];
      $j++;
    } else {
      $response[$i]["eventName"] = $name;
      $response[$i]["eventId"] = $brow["id"];
      $response[$i]["startDate"] = $brow["startDate"];
      $response[$i]["endDate"] = $brow["endDate"];
      $response[$i]["publishedAt"] = $brow["publishedAt"];
      $response[$i]["price"] = $brow["price"];
      $response[$i]["tickets"] = $brow["tickets"];
      $i++;
    }
  }
  if (0 < count($endedEvents)){
   $response = array_merge($response,$endedEvents);
  }
}
...
<body>
      ...
      <div class="navbar navbar-primary">
        ...
        <div class="navbar-inner container">
          <ul class="nav-primary">
            ...
            <li>
              <a href="/admin/opret/" class="nav-link" title="Opret event">
                <span>Opret begivenhed</span>
              </a>
            </li>
            <li class="current">
              <a href="#" class="nav-link" title="Fester">
  ...
  <main class="container page-container">
    <section class="new-events">
      <h1 class="h2"> Mine planlagte begivenheder</h1>
      <p class="font-lead">Her er et overblik over alle de kommede begivenhedder, som du administrer.</p>
      <div class="row">
        <?php 
        $newEvents = true;
        for ($i = 0; $i < count($response); $i++) {
          $publishInfo="";
         if ($response[$i]["publishedAt"] < time()){
            $publishInfo = '<a style="text-align:left; text-decoration:none" href="https://gymfest.mitstudiekort.dk/admin/events/event?id='.$response[$i]["eventId"].'"><p class="px-4"><strong>Offentliggjort den: </strong>'.date("d/m-Y H:i", substr($response[$i]["publishedAt"], 0, 10)).'</p></a>';
          } else{
            $publishInfo = '<a style="text-align:left; text-decoration:none" href="https://gymfest.mitstudiekort.dk/admin/events/event?id='.$response[$i]["eventId"].'"><p class="px-4"><strong>Offentliggøres den: </strong>'.date("d/m-Y H:i", substr($response[$i]["publishedAt"], 0, 10)).'</p></a>';
          }
          
          if ($response[$i]["endDate"] < time() && $newEvents){
            echo '
            </div>
            <div class="row" style="display:flex; place-content:center;" class="p-4" id="eventSeperator">
              <button class="button button-unstyled effect-underline" style="display: flex; flex-flow: wrap;" id="endedButton">
              Afsluttede begivenhedder 
              <svg style="width: 40px; height:30px;" class="icon-svg" focusable="false" aria-hidden="true">
              <use xlink:href="#angle-arrow-down"></use>
              </svg>
              </button>
              </div>
              <div class="row endedEvents" id="endedEvents">
              ';
            $newEvents = false;
          }
                echo  '
                <div class="col-12 col-sm-6 col-md-4 col-lg-4 pb-4">
                  <div class="card card-align-height" style="border-radius: 10px; ',($response[$i]["endDate"] < time() ? 'background-color: #F9FAFB' : '').'">
                    <div class="card-header" style="padding: 0;',($response[$i]["endDate"] < time() ? 'background-color: #F9FAFB' : '').'">
                    <a href="https://gymfest.mitstudiekort.dk/admin/events/event?id='.$response[$i]["eventId"].'"> <img src="/assets/rave-example-photo.jpg" style="height: 176px; width:100%; object-fit: cover;',($response[$i]["endDate"] < time() ? 'filter: grayscale(100%);' : '').'"/> </a>
                    </div>
                    <div class="card-content style="">
                      <div class="content">
                        <div class="row">
                          <div class="col-12">
                          <a style="text-decoration:none" href="https://gymfest.mitstudiekort.dk/admin/events/event?id='.$response[$i]["eventId"].'"><p class="h4 py-4">'.$response[$i]["eventName"].'</p></a>
                          '.$publishInfo.'
                          <a style="text-align:left; text-decoration:none" href="https://gymfest.mitstudiekort.dk/admin/events/event?id='.$response[$i]["eventId"].'"><p class="px-4"> <strong>Starter den:</strong> '. date("d/m-Y H:i", substr($response[$i]["startDate"], 0, 10)) .' </p></a>
                          <a style="text-align:left; text-decoration:none" href="https://gymfest.mitstudiekort.dk/admin/events/event?id='.$response[$i]["eventId"].'"><p class="px-4"> <strong>Pris pr. billet:</strong> '.$response[$i]["price"].' kr.</p></a>
                          <a style="text-align:left; text-decoration:none" href="https://gymfest.mitstudiekort.dk/admin/events/event?id='.$response[$i]["eventId"].'"><p class="px-4"> <strong>Antal billetter:</strong> '.$response[$i]["tickets"].'</p></a>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div class="card-footer" style="padding: 20px; display: flex; justify-content:space-evenly !important; ">
                      <a class="button ',($response[$i]["endDate"] < time() ? 'button-secondary' : 'button-primary').' alert-leave d-print-none" href="https://gymfest.mitstudiekort.dk/admin/events/event?id='.$response[$i]["eventId"].'">
                        Gå til eventet
                      </a>
                    </div>
                  </div>
