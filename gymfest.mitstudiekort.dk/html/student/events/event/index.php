...
if ($eventID != null) {
  $sql = "SELECT * FROM `events` INNER JOIN invitations ON events.id = invitations.eventId WHERE invitations.userId = '".($user->id)."' AND events.id = '".($eventID)."'";
  ...
  if ($result->num_rows > 0) {
    while($brow = $result -> fetch_assoc()) {
      if ($user->id == $brow["studentId"]) {
        $isAdmin = true;
        $studentEditButton = '<a class="button button-primary alert-leave d-print-none my-2" id="edit-button" style="width: 100%;" href="/student/events/event/edit?id='.$eventID.'">Rediger event</a>';
      }
    }
  }
}
...
if ($eventID != null) {
  $sql = "SELECT userId, type FROM `tickets` WHERE eventId = '".($eventID)."'";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    $studentBuyButton = '<a class="button button-primary alert-leave d-print-none my-2" id="checkout-button" style="width: 100%;margin-left: 0px;" href="#">Tilkøb plusone</a>';
    $onlyPlusOne = 1;
    while($brow = $result -> fetch_assoc()) {
      if ($brow["type"] == 'PLUSONE') {
        $studentBuyButton = '<a class="button button-primary alert-leave d-print-none my-2" id="" style="width: 100%;margin-left: 0px;background-color:grey" href="#">Du har allerede en billet</a>';
      }
    }
  } else {
    $studentBuyButton = '<div class="form-group" style="margin-top:0px;"><a class="button button-primary alert-leave d-print-none my-2" id="checkout-button" style="width: 100%;margin-left: 0px;" href="#">Køb billet</a><input id="plusOneCheck" type="checkbox" name="checkbox-large[]" value="1" class="form-checkbox  checkbox-large "><label for="plusOneCheck" class="form-label">Plus one?</label></div>';
  }
}
...
<!-- GYMFEST -> Student -> EVENTS -> Event -> id -->
      ...
      <div class="event-header col-lg-10 offset-lg-1 col-md-10 offset-md-1 col-sm-12">
        ...
        <div class="mx-5 py-5 header-details">
          ...
          <div class="header-column-2">
            <?php echo $studentEditButton ?>
            <?php echo $studentBuyButton ?>
          </div>
        </div>
      </div>
    </div>
...
<script type="text/javascript">
    ...
    checkoutButton.addEventListener("click", function () {
      ...
      fetch("https://gymfest.mitstudiekort.dk/payment/create-checkout-session.php?eid=<?php echo $eventID; ?>&opo=<?php echo $onlyPlusOne ?>&po="+plusOne, {
        method: "POST",
      })
