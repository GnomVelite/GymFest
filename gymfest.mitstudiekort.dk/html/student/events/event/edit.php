...
if ($eventID != null) {
  $sql = "SELECT studentId FROM `studentEdits` WHERE eventId = '".($eventID)."'";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    while($brow = $result -> fetch_assoc()) {
      if ($user->id == $brow["studentId"]) {
        $isAdmin = true;
      }
    }
  }
...
if ($redirect){
  header_remove('Location');
  header('Location: /student/events');
}
?>
<!doctype html>
...
<body>
      ...
      <div class="navbar navbar-primary">
          ...
          <div class="navbar-inner container">
            <ul class="nav-primary">
              ...
              <li class="current">
                <a href="/student/events/" class="nav-link" title="Min profil">
                  <span>Begivenheder</span>
                </a>
              </li>
              <li>
                <a href="/student/tickets" class="nav-link" title="Fester">
                  <span>Billeter oversigt</span>
                </a>
              </li>
            </ul>
          </div>
  ...
  <div id="desktop-only">
    <main class="container page-container" style="padding-left: 0; padding-right: 0; max-width: none">
      <div class="event-fader col-lg-12 col-md-12 col-sm-12 mb-2"
        style="max-width: none; background: rgb(255,255,255); background: linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(242,241,241,1) 100%); padding:none;">
        <div class="event-header col-lg-10 offset-lg-1 col-md-10 offset-md-1 col-sm-12">
          <img src="/assets/rave-example-photo.jpg" class="mx-auto header-img" alt="Dit event foto">
          <div class="mx-5 py-5 header-details">
            <div class="header-column-1">
              <div class="form-group" style="max-width: 25rem;">
                ...
                <input class="form-input mb-4" id="event-name" value="<?php echo $eventName?>" name="event-name" />
                <!-- <h1 class="h1" style="margin: 0; text-align: start;margin-bottom:5px"></h1> -->
              </div>
            </div>
                ...
                <div>
                  ...
                  <div onclick="showCal(date1);" class="date-1" style="display:flex">
                    <input type="text" onclick="showCal(date1);" id="event-start-date" 
                      style="border: 1px solid #747474; border-radius: 4px; color: #1a1a1a; display: block; font-size: 1.6rem; line-height: 2.6rem; padding: calc(8px - 1px) calc(16px - 1px); width: 100%; max-width: 32rem; margin-top: 8px; text-align: left;" disabled>
                  </div>
                </div>
                <h2 style="width: 30px; text-align:center;"> - </h2>
                <div>
        ...
        <div class="row">
          <div class="col-12 col-sm-12 col-md-6 col-lg-6">
            <div class="card mb-5" style="border-radius: 10px">
              ...
              <div class="content"
                style="background-color: #e8e9f3; border-color: black; border-width: 1px; border-top: 1px solid #d8d8d8;">
                        ...
                        <div class="mt-2" style="display: flex; flex-direction:column;">
                          <div class="mt-4" style="display: flex; flex-direction:row; align-items:center;">
                            <div style="height: 3rem; width:3rem;">
                              <svg style="width:100%; height:100%;" class="icon-svg" focusable="false"
                                aria-hidden="true">
                          ...
                          <div class="form-group" style="max-width: 25rem;">
                            ...
                            <input class="form-input mb-4" id="ticket-price" value="<?php echo $price?>"
                              name="ticket-price" type="number" disabled/>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div class="card mb-5" style="border-radius: 10px">
              ...
              <div class="content"
                style="background-color: #e8e9f3; border-color: black; border-width: 1px; border-top: 1px solid #d8d8d8; height: 60vh;">
          ...
          <div class="col-12 col-sm-12 col-md-6 col-lg-6 mt-6 mt-md-0">
            <div class="card mb-5" style="border-radius: 10px">
              <div class="card-header">
                <p class="h3">Redigering</p>
              </div>
              <div class="content"
                style="background-color: #e8e9f3; border-color: black; border-width: 1px; border-top: 1px solid #d8d8d8; height: 150px;">
                <div class="row justify-content-center">
                  <div class="col-12">
                    <div class="row bg-violet p-5">
                      <div class="col-12 align-text-left">
                        <button class="button button-primary alert-leave d-print-none my-2"
                          style="width: 100%; margin-left:0; display: flex;flex-direction: row;justify-content: center;align-items: center;" onclick="handleEdit()" id="save-edit-button">
                          Gæm ændringer
                        </button>
                        <a class="button button-danger alert-leave d-print-none my-2" style="width: 100%; margin-left:0"
                          href="/admin/events/event?id=<?php echo $eventID ?>">
                          Slet ændringer
                        </a>
                      </div>
                    </div>
            ...
            <div class="card mb-5" style="border-radius: 10px">
              ...
              <div class="content"
                ...
                <div class="row justify-content-center">
                  <div class="col-12">
                    <!-- Alt i denne div er blot et eksempel. Visse klasser er ikke en del af FDS. -->
                    <div class="row bg-violet px-5 pb-5 pt-3" style="height: 540px;;">
                      <div class="col-12 align-text-left">
                        <div id="editor-container" class="ql-editor"
                          style="min-height: 250px; max-height: 435px; white-space: normal; padding: 0; background:white;">
      ...
      <div class="fds-modal" id="location-modal" aria-hidden="true" role="dialog" aria-modal="true"
        ...
        <div class="modal-content" style="height:50%; width:80%;">
          <div class="modal-header">
            ...
            <button class="modal-close button button-unstyled" aria-label="Luk" data-modal-close=""><svg
                class="icon-svg" focusable="false" aria-hidden="true">
                <use xlink:href="#close"></use>
              </svg></button>
          </div>
          ...
          <div class="modal-footer">
            <button class="button button-primary" data-modal-close="" onclick="updateMap()">Gem
              placering</button>
  ...
  <script>
    $('document').ready(function () {
      console.log("<?php echo (date("d/m-Y H:i", substr($startDate, 0, 10)));?>")
      date1 = flatpickr("#event-start-date", {
        ...
        defaultDate: "<?php echo (date("d/m-Y H:i", substr($startDate, 0, 10)));?>"
      ...
      date2 = flatpickr("#event-end-date", {
        ...
        time_24hr: true,
        dateFormat: "d/m-Y H:i",
        defaultDate: "<?php echo (date("d/m-Y H:i", substr($endDate, 0, 10)));?>"
      });
    });
  ...
  <script>
    ...
    xhr.open("GET",
      "https://api.dataforsyningen.dk/adgangsadresser/reverse?x=<?php echo $lng ?>&y=<?php echo $lat ?>&struktur=mini&valider"
    );
    ...
    function updateMap() {
      lattitude = newSelected.data.y;
      longitude = newSelected.data.x;
      ...
      var text = document.createElement('text');
      text.textContent = newSelected.tekst;
      infowincontent.appendChild(text);
    ...
    dawaAutocomplete.dawaAutocomplete(document.getElementById('dawa-autocomplete-input'), {
      select: function (selected) {
        console.log('Valgt adresse: ' + selected.tekst + ' Valgt id: ' + selected.data.x + selected.data.y);
        //updateMap(selected.data.y, selected.data.x, selected.tekst);
  ...
  <script async
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCXYCbnfIMcYFMU70o6W7iVvQchKkHKRoI&callback=initMap">
  </script>
  <script>
    const validateForm = () => {
      ...
      if (eventName.length > 0 && startTime.length > 0 && endTime.length > 0 &&
    ...
    function handleEdit() {
      ...
      if (validateForm()) {
        ...
        $("#save-edit-button").html(
          '<div style="font-size:6px; text-align:center" class="spinner-white"> </div>'
        );
        delta = delta.replace("&", "§€§€");
        var startDateUnix = moment(startTime, "D/M-YYYY H:mm").unix();
        var endDateUnix = moment(endTime, "D/M-YYYY H:mm").unix();
        console.log(eventName, startDateUnix, endDateUnix, price, delta, lattitude, longitude, eventID);

        var xhttp = new XMLHttpRequest();
        var url = 'https://gymfest.mitstudiekort.dk/api/onsite/event/handleEditEvent.php';
        var xhttpError = "";
        ...
        xhttp.onreadystatechange = function () {
          if (this.readyState == 4 && this.status == 200) {
            ...
            setTimeout(() => {
              window.location.replace("/student/events/event?id=<?php echo $eventID ?>");
            }, 700);
          }
        }

        xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhttp.send("se=" + startDateUnix + "&ee=" + endDateUnix + "&en=" + eventName + "&p=" + price +
          "&d=" + delta + "&lng=" + longitude + "&lat=" + lattitude + "&id=" + eventID);
      }
    }
  </script>
</body>