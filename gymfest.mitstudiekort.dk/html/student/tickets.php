...
// SELECT tickets.ticketToken, events.name FROM ((tickets INNER JOIN payments ON payments.stripe_id = tickets.stripe_id ) INNER JOIN events ON events.id = tickets.eventId ) WHERE payments.status = 'checkout.session.completed' AND tickets.userId = 28 ORDER BY events.startDate ASC
$sql = "SELECT tickets.ticketToken, events.name FROM ((tickets INNER JOIN payments ON payments.stripe_id = tickets.stripe_id ) INNER JOIN events ON events.id = tickets.eventId ) WHERE payments.status = 'checkout.session.completed' AND tickets.status = 'VALID' AND tickets.userId = ".($user->id)." ORDER BY events.startDate DESC";
...
if($result->num_rows > 0) {
  ...
  while($row = $result -> fetch_assoc()) {
    $token = mb_convert_encoding($row["ticketToken"], "UTF-8");
    $name = mb_convert_encoding($row["name"], "UTF-8");

    $response[$i]["ticketToken"] = $token;
...
<head>
  ...
  <link type="text/css" rel="stylesheet" href="ticket.module.css" />
  <link type="text/css" rel="stylesheet" href="gallery.module.css" />
...
<body>
  ...
  <?php 
    if (count($response) == 0){
      echo '<main class="container page-container"><h2 style="text-align: center;">Du har ikke købt nogle billetter endnu</h2>
      <div class="mt-7 t-container" style=" transform: rotate(0deg);">
              <div class="card card">
                <h2></h2>
                <i class="fas fa-arrow-right"></i>
                <p></p>
                <div class="pic">
                  <div style="height: 8rem; width: 8rem;position: absolute;top: 35%;right: 42%;">
      ...
      echo'<div class="gallery js-flickity" data-flickity-options=\'{ "wrapAround": false }\'>';
        for ($i = 0; $i < count($response); $i++){

      $split = str_split($response[$i]["ticketToken"], 3);
      $ticketID = implode('-', $split);
    ...
    echo '
       <div class="gallery-cell">
            <div class="t-container">
              <div class="card card2">
                ...
                <div class="pic" style="background-image: linear-gradient(-90deg, rgba(70,92,228,0.53) 0%, rgba(168,25,200,0.9) 100%), url(\'https://images.unsplash.com/photo-1525543907410-b2562b6796d6?ixlib=rb-0.3.5&s=9ff8e5e718a6a40cbd0e1471235912f4&auto=format&fit=crop&w=3452&q=80\');">
                </div>
                <ul>
                  <li></li>
                  <li></li>
      ...
      echo'</div> <script src="https://unpkg.com/flickity@2/dist/flickity.pkgd.min.js"></script>'
      ?>
