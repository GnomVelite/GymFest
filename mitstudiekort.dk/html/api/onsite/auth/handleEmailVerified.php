...
$keyCorrect = "false";
if (is_string($key) && (preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/', $key))) {
  $isUUID = true;
  $keyCorrect = "true";
} else{
  $isUUID = false;
...
if ($isUUID){
$userSql = "SELECT emailDate, id, passwordKey, loginChange, email FROM users WHERE `emailKey` = '".($key)."'";
$userResult = $conn->query($userSql);
  ...
  //hvis der er gået mindre end 1 dag & brugeren ikke har en passwordkey
  if ($time < $date + (24 * 60 * 60)){
    if ($passwordKey == NULL){
      if (!$loginChange == NULL){ // bestem om der skal ændres på login eller verificeres allerede kendt email
      $sql = "UPDATE users SET `emailVerifiedAt`= '".($time)."', emailKey = NULL, emailDate = NULL, loginChange = NULL, email = '".($loginChange)."' WHERE `id` = '".($id)."'";
      } else{
        $sql = "UPDATE users SET `emailVerifiedAt`= '".($time)."', emailKey = NULL, emailDate = NULL WHERE `id` = '".($id)."'";
      }
...
<head>
  ...
  <link type="text/css" rel="stylesheet" href="/vendor/circleLoader.module.css" />
...
<body>
  ...
  <main class="container page-container">
    ...
    <div class="row justify-content-center mb-2">
      <div class="col-12">
        <div class="h2" style="text-align: center;">Mit Studiekort</div>
        <h1 class="h2" style="font-size:20px; max-width: 100%;">Validerer Email</h1>
  ...
  <script>
    function toShow() {
      var result = <?php echo $verified ?>;
      var inCorrectKey = <?php echo $keyCorrect ?>;
      var inTime = <?php echo $pastDue ?>;
      setTimeout(function () {
        if (result) {
          statusChange('success');
          $("#userMessage").html(
              '<div class="h2"style="color:#5cb85c; font-size:20px; max-width: 100%;" id="failed">Din email blev verificeret.</div>'
              );
        } else {
          statusChange('failed');
          if (!inCorrectKey) {
            $("#userMessage").html(
              '<div class="h2"style="color:red; font-size:20px; max-width: 100%;" id="failed">Du har ikke adgang til denne side</div>'
              );
          } else if (inTime){
            $("#userMessage").html(
              '<div class="h2"style="color:red; font-size:20px; max-width: 100%;" id="failed">Dit link er desværre løbet ud, prøv igen.</div>'
              );
          }else {
            $("#userMessage").html(
              '<div class="h2"style="color:red; font-size:20px; max-width: 100%;" id="failed">Noget gik galt, prøv igen.</div>'
              );
          }
        }
        redirect();
    ...
    function redirect(){
      setTimeout(function(){
        window.location.href = "https://www.mitstudiekort.dk";
      }, 1500);
    }
  </script>
  <script>
