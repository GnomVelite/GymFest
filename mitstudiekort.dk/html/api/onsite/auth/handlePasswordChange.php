...
if ($isUUID){
$userSql = "SELECT emailDate, id, emailKey, loginChange FROM users WHERE `passwordKey` = '".($key)."'";
$userResult = $conn->query($userSql);
  ...
  if ($time < $date + (24 * 60 * 60)){
    if ($emailKey == NULL && $loginChange != NULL){
      $sql = "UPDATE users SET `password`= '".($loginChange)."', passwordKey = NULL, emailDate = NULL, loginChange = NULL WHERE `id` = '".($id)."'"; 
  ...
  <main class="container page-container">
    ...
    <div class="row justify-content-center mb-2">
      <div class="col-12">
        ...
        <h1 class="h2" style="font-size:20px; max-width: 100%;">Ændrer kodeord</h1>
    ...
    function toShow() {
      ...
      setTimeout(function () {
        if (result) {
          ...
          $("#userMessage").html(
              '<div class="h2"style="color:#5cb85c; font-size:20px; max-width: 100%;" id="failed">Dit kodeord blev ændret.</div>'
              );
        } else {
          statusChange('failed');
          if (!inCorrectKey) {
